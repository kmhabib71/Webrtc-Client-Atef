...
const urlParams = new URLSearchParams(window.location.search);
var FileSenderUsername = urlParams.get("username");

function sentFileFunc() {
  var fileeName = $("#customFile").val().split("\\").pop();
  alert(fileeName);
  var FileePath =
    "./../../public/attachment/" + FileSenderUsername + "/" + fileeName + "";
  send({
    type: "filee",
    name: FileSenderUsername,
    FileePath: FileePath,
    fileeName: fileeName,
  });
  chatArea.innerHTML +=
    "<div class='left-align' style='display:flex; align-items:center;align-self:flex-end;'><img src='assets/images/other.jpg' style='height:40px;width:40px;' class='caller-image circle'><div style='font-weight:600;margin:0 5px;'>" +
    FileSenderUsername +
    "</div>: <div><a href='" +
    FileePath +
    "' download>" +
